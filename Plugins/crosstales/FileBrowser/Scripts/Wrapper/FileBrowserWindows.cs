...
namespace Crosstales.FB.Wrapper
{
   ...
   public class FileBrowserWindows : BaseFileBrowser
   {
      ...
      public override string[] OpenFolders(string title, string directory, bool multiselect)
      {
         ...
         if (!Util.Helper.isIL2CPP && !FileBrowser.Instance.LegacyFolderBrowser)
            Debug.Log("The new folder dialog works only with IL2CPP!");

         string[] paths = null;

         if (!Util.Helper.isIL2CPP || FileBrowser.Instance.LegacyFolderBrowser)
         {
            paths = openFolders(directory, false);
         }
         else
         {
            paths = openFoldersNew(directory, false);
         }
      ...
      public override void OpenFoldersAsync(string title, string directory, bool multiselect, Action<string[]> cb)
      {
         ...
         //if (Util.Helper.isIL2CPP || !FileBrowser.Instance.LegacyFolderBrowser)
         //   Debug.Log("The new folder dialog works only with sync-calls under IL2CPP!");

         if (!Util.Helper.isIL2CPP || FileBrowser.Instance.LegacyFolderBrowser) // || isFirsttime)
         {
      ...
      private static string[] openFiles(string title, string directory, string defaultName, bool multiselect, params ExtensionFilter[] extensions)
      {
         try
         {
            ...
            if (NativeMethods.GetOpenFileName(ref ofn))
            {
               ...
               instance.CurrentOpenSingleFile = Util.Helper.ValidateFile(file);
...
*/
         ...
         catch (Exception ex)
         {
            ...
            return openFolders(directory, isAsync); //TODO find a better solution for "System.Runtime.InteropServices.COMException (0x8001010E): Error HRESULT E_FAIL has been returned from a call to a COM component."
         }
         finally
         {
